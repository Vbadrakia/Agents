...
def api_ai_analysis():
    ...
    # If explicit manual refresh is requested OR cache hasn't been created yet
    if force or not os.path.exists(cache_file):
        ...
        try:
            ...
            data += f"\n\n🕒 Live Report generated by Agent at {dt_str} (Auto-updates every 15 mins)"
            
            return Response(data, mimetype="text/plain")
        except Exception as e:
            return Response(f"Error loading AI analysis: {str(e)}", mimetype="text/plain")

    # Background cache mode (File exists)
